...
{
    public partial class CEOApproval : System.Web.UI.Page
    {
            ...
            if (!IsPostBack)
            {
                ...
                if (Session["UserInfo"] != null)
                {
                    ...
                    if (Request.QueryString["SRNO"] != null)
                    {
                        ...
                        btnFinanceSubmit.Enabled = false;
                        ...
                        SR_AccontsApprovalData(Request.QueryString["SRNO"]);
        ...
        private void SR_AccontsApprovalData(string SRNO)
        {
            try
            {


            DataTable dtAccApproveData = new DataTable();
            objBO.SR_No = SRNO;


            dtAccApproveData = objBLL.GetAccontsApprovalData_CEO(objBO);

            if (dtAccApproveData.Rows.Count > 0)
            {
                txtFinanceComments.Text = dtAccApproveData.Rows[0]["ACCCOMMENTS"].ToString();

                chkFinanceApprove.Checked = true;

                byte[] bytes = (byte[])dtAccApproveData.Rows[0]["ACCSIGNCONTENT"];
                string base64String = Convert.ToBase64String(bytes, 0, bytes.Length);

                imgFinanceSignature.ImageUrl = "data:image/png;base64," + base64String;
        ...
        protected void btnCEOSubmit_Click(object sender, EventArgs e)
        {
            try
            {


            if (txtCEOComments.Text == "")
            {
            ...
            if (chkCEOApprove.SelectedIndex == 0)
            {
                objBO.CEOApprovalChkFlag = "A";
            }
            if (chkCEOApprove.SelectedIndex == 1)
            {
                objBO.CEOApprovalChkFlag = "R";
            }

            string Result = string.Empty;

            objBO.SR_No = txtSRNo.Text;
            objBO.CEOApprovalComments = txtCEOComments.Text;
            objBO.UserID = objUserInfo.UserID;

            Result = objBLL.InsertCEOApproval(objBO);
        ...
        {
            imgCEOSignature.ImageUrl = "data:image/png;base64," + (string)ViewState["Sign"];

        }

       

        protected void chkCEOApprove_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (chkCEOApprove.SelectedIndex == 0)
            {
                BindSignature();
            }
            else if (chkCEOApprove.SelectedIndex == 1)
            {
